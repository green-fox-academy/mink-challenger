...
public class CommitmentController {
  ...
  public String addCommitment(@PathVariable long userId, String description, String date, Long challengeId){
  ...
  public String renderEditCommitmentPage (Model model, @PathVariable long commitmentId){
    ...
    model.addAttribute("commitments",commitmentService.findAllByUser(commitment.getUser().getId()));
    model.addAttribute("currentCommitment", commitment);
    return "editcommitment.html";
  }

  @PostMapping("/saveCommitment/{commitmentId}")
  public String editCommitment(@ModelAttribute("currentCommitment") Commitment currentCommitment, @PathVariable long commitmentId){
    commitmentService.saveChangedCommitment(commitmentId, currentCommitment);
    return "redirect:/commitment/" + commitmentService.getCommitmentById(commitmentId).getUser().getId();
  }
}